...
public class PlayerController : MonoBehaviour {
    ...
    public bool moveTowardsX(float inputPos) {
        /* 
        commented lines were meant to reduce iterations 
        of unnecessarily assigning velocity to player 
        (incomplete implementation)
        */
        bool direction = inputPos > transform.position.x;
        if (
            direction == transform.position.x > 0 && 
            Mathf.Abs(transform.position.x) > Player.instance.Xconstraint
        ) {
            return false;
        }
